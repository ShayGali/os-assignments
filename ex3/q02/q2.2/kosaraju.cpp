...
vector<vector<int>> gen_adj_matrix(int n, int seed = 0) {
    ...
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < n; j++) {
            if (i != j)
                adj_matrix[i][j] = (rand() % 2) * (rand() % 2)* (rand() % 2);
        }
    }
...
int main() {
    int n = 10;
    cout << "gen_adj_list (p = 0.125)\n";
    vector<vector<int>> adj_list = gen_adj_list(n,3);


    cout << "gen_adj_matrix (p = 0.125)\n";
    vector<vector<int>> adj_matrix = gen_adj_matrix(n,3);
    ...
    cout << "\nkosaraju_adj_mat\n";
    components = kosaraju_adj_mat(adj_matrix);
